...
public class XPBar : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            preview.fillAmount += previewFillSpeed * Time.unscaledDeltaTime * GetFillBonus();
        }
        
        if (preview.fillAmount > amount)
        {
            preview.fillAmount -= previewFillSpeed * 2 * Time.unscaledDeltaTime * GetFillBonus();
        }
    }
    ...
    private IEnumerator Fill()
    {
        while (true)
        {
            while (toFill.Count > 0)
            {
                ...
                {
                    slider.value += Time.unscaledDeltaTime * fillSpeed * GetFillBonus();
