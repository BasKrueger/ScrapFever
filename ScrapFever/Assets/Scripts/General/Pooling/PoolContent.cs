...
public class PoolContent 
{
    ...
    {
        var key = $"{typeof(T).ToString()}_{name}";

        if (isPooling.Keys.Contains(key))
        {
            return isPooling[key];
        }
        ...
        if (name == "")
        {
            result = objectToPool.GetComponent<T>() != null;
        }
        else
        {
            result = objectToPool.GetComponent<T>() != null && objectToPool.name == name;
        }

        isPooling.Add(key, result);
        return result;
    }

    public bool IsPooling(GameObject template) => objectToPool == template;
